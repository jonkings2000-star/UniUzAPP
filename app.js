...
function chooseFaculty(uni){app.innerHTML=`<div class="wrap">${brand()}<div class="card"><h2>${tr("faculty")}</h2><input id="fac" class="input" placeholder="${tr("facultyPlaceholder")}"><button class="btn primary" onclick="chooseGroup('${esc(uni)}')">${tr("continue")}</button></div></div>`}
function chooseGroup(uni){const fac=document.getElementById("fac").value.trim();if(!fac)return toast(tr("faculty"));app.innerHTML=`<div class="wrap">${brand()}<div class="card"><h2>${tr("group")}</h2><input id="grp" class="input" placeholder="AI-101"><button class="btn primary" onclick="chooseName('${esc(uni)}','${esc(fac)}')">${tr("continue")}</button></div></div>`}
...
function home(){layout(`${brand()}<div class="card"><h2>👋 ${esc(profile?.first_name||"")}</h2><p class="muted">${esc(profile?.university||"")} · ${esc(profile?.department||"")} · ${esc(profile?.group_name||"")}</p></div>
<div class="grid"><button class="btn" onclick="showSchedule()">🗓️ ${tr("schedule")}</button><button class="btn" onclick="showHomework()">📝 ${tr("homework")}</button><button class="btn" onclick="showAI()">🤖 ${tr("ai")}</button><button class="btn" onclick="showReminders()">🔔 ${tr("reminders")}</button><button class="btn" onclick="showProfile()">👤 ${tr("profile")}</button>${(profile?.is_admin || profile?.telegram_id===(tg?.initDataUnsafe?.user?.id))?`<button class="btn" onclick="showAdmin()">🔐 ${tr("admin")}</button>`:""}</div>`)}
...
async function showHomework(){try{const d=await api("/homework");layout(`${brand()}<div class="card"><h2>📝 ${tr("homework")}</h2><input id="ht" class="input" placeholder="${tr("title")}"><textarea id="hd" placeholder="${tr("description")}"></textarea><input id="hdate" class="input" type="datetime-local"><input id="hfile" class="input" type="file" accept=".pdf,image/*,.doc,.docx"><button class="btn primary" onclick="addHW()">${tr("add")}</button></div><div class="card">${d.items.length?d.items.map(x=>`<div class="list-item"><div class="row"><b>${esc(x.title)}</b><span class="badge">${esc(x.due_at)}</span></div><p class="muted">${esc(x.description||"")}</p><div class="row"><button class="btn" onclick="completeHW(${x.id})">${tr("done")}</button><button class="btn danger" onclick="deleteHW(${x.id})">${tr("delete")}</button></div></div>`).join(""):`<div class="empty">${tr("noData")}</div>`}</div>`,"homework")}catch(e){toast(e.message)}}
...
async function completeHW(id){try{await api(`/homework/${id}/complete`,{method:"POST"});showHomework()}catch(e){toast(e.message)}}
async function deleteHW(id){try{await api(`/homework/${id}`,{method:"DELETE"});showHomework()}catch(e){toast(e.message)}}

function showAI(){layout(`${brand()}<div class="card"><h2>🤖 ${tr("ai")}</h2><div id="chat" class="card"></div><textarea id="q" class="input" placeholder="${tr("question")}"></textarea><button class="btn primary" onclick="askAI()">${tr("send")}</button></div>`,"ai")}
